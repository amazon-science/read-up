...
from src.model.self_play_base_oracle_other_vilbert import SelfPlayModel
...
from src.data.self_play_base_oracle_other_vilbert import SelfPlayDataset, collate_fn
...
class SelfPlaySolver(BaseSolver):
    ...
    def fetch_data(self, data):
        game, image_features_rcnn_qgen, bboxs_rcnn_qgen, \
            image_features_rcnn_gt_guesser, bboxs_rcnn_gt_guesser, tgt_img_feat, tgt_bbox_gw, tgt_cat, \
    ...
    def validate(self, specified_set):
        ...
        for val_step, data in enumerate(specified_set):
            ...
            game, qgen_img_feats, qgen_bboxs, image_features_rcnn_gt_guesser, bboxs_rcnn_gt_guesser, \
                 tgt_img_feat, tgt_bbox_gw, tgt_cat, cats_guesser, bboxs_mask, label, qs, q_len = self.fetch_data(data)
            with torch.no_grad(): 
                if self.use_gt_question:
                    # TODO:
                    pass
                    ...
                    pred, dialog, q_log, a_log, a_conf_log = self.model.play(
                        qgen_img_feats, qgen_bboxs, 
                        image_features_rcnn_gt_guesser, bboxs_rcnn_gt_guesser,
                        tgt_cat, tgt_bbox_gw, tgt_img_feat, cats_guesser, bboxs_mask,
                        self.tokenizer.sos_id, self.tokenizer.pad_id, 
                        self.tokenizer.eoq_id, self.tokenizer.eod_id,
                        self.answer2id, self.answer2token, max_q_len=20, max_turns=5,
                    )
