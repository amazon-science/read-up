...
from src.model.self_play_oracle_rcnn import SelfPlayModel
...
from src.data.self_play_oracle_rcnn import SelfPlayDataset, collate_fn
...
class SelfPlaySolver(BaseSolver):
    ...
    def fetch_data(self, data):
        game, img_feat, tgt_cat, tgt_bbox, tgt_img_feat, cats, bboxs, bboxs_mask, label, qs, q_len = data
        return (
            game,
    ...
    def train(self):
        ...
        while self.step < self.max_step:
            ...
            for data in self.train_set:
                game, img_feat, tgt_cat, tgt_bbox, tgt_img_feat, cats, bboxs, bboxs_mask, label, qs, q_len = self.fetch_data(data)
                NOT_IMPLEMENT_YET()
                self.timer.cnt('rd')
                # Forward
            ...
            with torch.no_grad():
                if self.use_gt_question:
                    pred, dialog = self.model.play_with_gt_question(
                        qs, q_len, tgt_cat, tgt_bbox, tgt_img_feat, cats, bboxs, bboxs_mask, 
                        self.tokenizer.pad_id, self.answer2id, self.answer2token)
                else:
                    pred, dialog, q_log, a_log, a_conf_log = self.model.play(
                        img_feat, tgt_cat, tgt_bbox, tgt_img_feat, cats, bboxs, bboxs_mask,
